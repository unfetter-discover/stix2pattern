...
def test_validate(client, pattern, validated):
    ...
    assert response.data.decode('utf8') == json.dumps(expectedValue, sort_keys=True)

# Test only the CAR-Elastic translation

CAR_ELASTIC_PASS = [
    ("[file:hashes.MD5 = '79054025255fb1a26e4bc422aef54eb4']",'data_model.object:file AND data_model.fields.md5_hash:"79054025255fb1a26e4bc422aef54eb4"'),
    ("[process:pid NOT IN (1, 2, 3) AND process:name = 'wsmprovhost.exe']",'(data_model.object:process AND data_model.fields.exe:\"wsmprovhost.exe\") AND (data_model.object:process AND NOT(data_model.fields.pid:(1 OR 2 OR 3)))'),
]
...
CAR_SPLUNK_PASS = [
    ("[process:pid NOT IN (1, 2, 3) AND process:name = 'wsmprovhost.exe']",'|where ((match(tag, \"dm-process-.*\") AND exe = \"wsmprovhost.exe\") AND (match(tag, \"dm-process-.*\") AND NOT (pid IN (1, 2, 3))))')
]
